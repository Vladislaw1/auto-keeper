import { useState, useRef, useContext, useEffect, useImperativeHandle } from 'react';
...
const DropdownMenu = (props: DropdownMenuProps & HTMLProps<HTMLElement>) => {
  ...
  const { floatingStyles, refs, context } = useFloating<HTMLElement>({
    ...
    placement: placement || (isNested ? getNestedDefaultPlacement() : getDefaultPlacement()),
    ...
    <FloatingNode id={nodeId}>
      {!isNested ? (
        ...
        <li
          ...
          {...toggleProps}
        >
          <DropdownSubItem>{renderTitle ? renderTitle : title}</DropdownSubItem>
        </li>
      )}
          ...
          {isMounted && (
            <FloatingPortal>
              ...
              >
                ...
                >
                  <ul
                    className={classNames('dropdown-menu min-w-[160px]', menuClass)}
                    style={styles}
                  >
                    {children}
                  </ul>
