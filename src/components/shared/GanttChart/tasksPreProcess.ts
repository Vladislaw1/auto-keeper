...
export default function tasksPreProcess(tasks: ExtendedTask[], colorsMap: Record<string, string>) {
    ...
    if (task.barVariant && colorsMap[task.barVariant]) {
      ...
      if (task.type === 'task') {
        task.styles = {
          ...
          backgroundSelectedColor: applyOpacityToRgb(color, 0.5) as string,
        };
      }
      ...
      if (task.type === 'project') {
        task.styles = {
          progressColor: applyOpacityToRgb('rgb(255,255,255)', 0.3) as string,
          backgroundColor: color,
          progressSelectedColor: applyOpacityToRgb('rgb(255,255,255)', 0.3) as string,
          backgroundSelectedColor: color,
        };
      }
    }
