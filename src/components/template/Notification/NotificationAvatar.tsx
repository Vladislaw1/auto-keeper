...
import { HiOutlineCalendar, HiOutlineClipboardCheck, HiOutlineBan } from 'react-icons/hi';
...
const GeneratedAvatar = ({ target }: { target: string }) => {
  ...
  return (
    <Avatar
      shape="circle"
      className={`text-gray-900 ${color(target)}`}
    >
      {acronym(target)}
    </Avatar>
  );
    ...
    case 2:
      return (
        <Avatar
          shape="circle"
          className={
            status === 'succeed' ? 'bg-emerald-200 text-gray-900' : 'bg-red-200 text-gray-900'
          }
