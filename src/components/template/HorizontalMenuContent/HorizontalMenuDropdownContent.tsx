...
const MenuIcon = ({ icon }: { icon: string }) => {
  ...
  return (
    <>
      {navigationIcon[icon] && (
        <span className={`text-xl`}>{navigationIcon[icon] || <TbCircle />}</span>
      )}
    </>
  );
};
...
const ColumnsLayout = (
  ...
  return (
    <div className="flex max-w-[1400px] w-full">
      <div className={classNames('grid gap-y-6 gap-x-8 p-6 flex-1', gridClasses[columns]?.grid)}>
              ...
              ?.subMenu.map((nav) => (
                ...
                >
                  ...
                  >
                    ...
                    >
                      <span className="text-xl">{navigationIcon[nav.icon] || <TbCircle />}</span>
                      <span>{t(nav.translateKey, nav.title)}</span>
                    </div>
                  </HorizontalMenuNavLink>
                </AuthorityCheck>
...
const HorizontalMenuDropdownContent = (props: HorizontalMenuDropdownContentProps) => {
