  ...
  return (
    ...
    >
      <div>
        {title &&
          typeof title === 'string' &&
          (customeHeader ? customeHeader() : <h3 className="font-bold">{title}</h3>)}
        <Suspense fallback={<></>}>
          {title && typeof title !== 'string' && <CustomHeader header={title} />}
        </Suspense>
      </div>
      <Suspense fallback={<></>}>
...
export const PageContainerBody = ({
  ...
  return pageContainerType === 'contained' ? (
    <Container className={classNames('h-full', className)}>{children}</Container>
  ) : (
    <>{children}</>
  );
};
...
const PageContainer = (props: PageContainerProps) => {
  ...
  const pageContainerDefaultClass = 'page-container relative h-full flex flex-auto flex-col';
